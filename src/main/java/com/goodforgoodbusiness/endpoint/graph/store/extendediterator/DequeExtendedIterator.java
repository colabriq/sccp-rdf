package com.goodforgoodbusiness.endpoint.graph.store.extendediterator;
...
import com.goodforgoodbusiness.endpoint.graph.store.extendediterator.functional.FilteredExtendedIterator;
import com.goodforgoodbusiness.endpoint.graph.store.extendediterator.functional.MappedExtendedIterator;
...
public class DequeExtendedIterator<T> implements ExtendedIterator<T> {
	...
	public void skip(T element) {	
		...
		if (seen.contains(element)) {
			// this causes problems with certain SPARQL queries and may not be part of the API contract anyway
